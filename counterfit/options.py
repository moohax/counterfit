class CFOptions:
    ...
    def __init__(self, attack_parameters):
        ...
        if "targeted" in self.attack_parameters.keys():
            self.attack_parameters["target_labels"] = {
                "docs": "target labels for a targeted attack",
                "default": 0,
            }
    ...
    def update(self, parameters_to_update: dict) -> None:
        ...
        for k, v in parameters_to_update.items():
            if k in self.attack_parameters.keys():
                self.attack_parameters[k]["previous"].append(
                    self.attack_parameters[k]["current"]
                )
...
def set_options(attack_parameters):
    CFOptions(attack_parameters)
