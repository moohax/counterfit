...
# https://github.com/monoxgas/sRDI/tree/master/Python
class sRDI:
    __version__ = "1.2"
    MACHINE_IA64 = 512
    MACHINE_AMD64 = 34404
    ...
    def doit(
        ...
        if obfuscate_imports:
            flags = flags | 0x4 | import_delay << 16

        if pass_shellcode_base:
            flags |= 0x8

        converted_dll = cls.ConvertToShellcode(
            dll,
            cls.HashFunctionName(function_name),
            user_data.encode(),
            flags,
        )
        ...
        if output_format == "raw":
            ...
            converted_dll_text = "".join(
                [r"\x{}".format(str(format(c, "02x"))) for c in converted_dll]
            )
            ...
            print(
                "Output format {} not recognized. Use 'raw' or 'string'".format(
                    output_format
                )
            )

    @classmethod
    def ConvertToShellcode(cls, dllBytes, functionHash=0x10, userData=b"None", flags=0):
        ...
        if cls.is64BitDLL(dllBytes):
            ...
            if len(bootstrap) != bootstrapSize:
                raise Exception(
                    "x86 bootstrap length: {} != bootstrapSize: {}".format(
                        len(bootstrap), bootstrapSize
                    )
                )
    ...
    @classmethod
    def is64BitDLL(cls, bytes):
        header_offset = struct.unpack("<L", bytes[60:64])[0]
        machine = struct.unpack("<H", bytes[header_offset + 4 : header_offset + 4 + 2])[
            0
        ]
        if machine == cls.MACHINE_IA64 or machine == cls.MACHINE_AMD64:
            return True
        return False

    @classmethod
    def HashFunctionName(cls, name, module=None):
