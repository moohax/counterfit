...
class AuglyAttack:
    ...
    def generate(self, x: np.ndarray, **kwargs):
        ...
        return (
            np.array(results, dtype=x.dtype),
            np.array(all_scores).reshape((x.shape[0], query_budget)).mean(axis=-1),
        )


class AuglyModule(CFModule):
    def __init__(self):
        super().__init__()

    @classmethod
    def get_attacks(
        cls, framework_path=f"{pathlib.Path(__file__).parent.resolve()}/attacks"
    ):
        attacks = {}
    ...
    def build(self, target: CFTarget, attack: str):
        ...
        new_attack = AuglyAttack(target.predict_wrapper, loaded_attack)
