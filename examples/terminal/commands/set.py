...
def get_sample_index(sample_index: str) -> Union[list, int, range, None]:
    try:
        ...
        print(f"Error parsing '--sample_index {sample_index}: {e}")
        return None
    ...
    if type(sample_index) not in (range, int, list):
        print(
            f"Error parsing '--sample_index {sample_index}: expression must result in a 'list', 'range' or 'int'"
        )
        return None
    ...
    if type(sample_index) is list:
        if any([type(el) is not int for el in sample_index]):
            print(
                f"Error parsing '--sample_index {sample_index}': list must only contain integers"
            )
            return None
...
def get_clip_values(clip_values: str) -> Union[tuple, NoneType]:
    try:
        ...
        print(f"Error parsing '--clip_values {clip_values}': {e}")
        return None
    ...
    if type(clip_values) not in (tuple,):
        print(
            f"Error parsing '--clip_values {clip_values}: expression must result in a 'tuple' or 'None'"
        )
        return None
...
def parse_numeric(argname: str, val_str: str) -> Union[int, float, None]:
    ...
    try:
        ...
        print(f"Error parsing --'{argname} {val_str}': {e}")
        return None

    if type(val) not in (int, float):
        print(
            f"Error parsing '--{argname} {val_str}': expression must result in a 'int' or 'float'"
        )
        return None
...
def parse_boolean(argname: str, val_str: str) -> Union[bool, None]:
    if val_str.lower() in ("true", "t", "yes", "y", "1"):
        ...
        print(f"Error parsing '--{argname} {val_str}': must be 'true' or 'false'")
        return None
...
def get_options() -> dict:
    ...
    if not CFState.state().active_target:
        ...
        options = {}
    elif CFState.state().active_target.active_attack and hasattr(
        CFState.state().active_target.active_attack, "options"
    ):
    ...
    for parameter, value in args.__dict__.items():
        if options.get(parameter):
            if parameter == "clip_values":
                ...
                type(options.get(parameter)["default"]) in (float, int)
