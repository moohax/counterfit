...
def do_predict(self, args: argparse.Namespace) -> None:
    ...
    if not target:
        print("No active target. Try 'interact' <target>")
        return
    ...
    if args.index is not None:  # default behavior
        ...
        if not active_attack:
            print("No active attack. Try 'use' <attack>")
            return
            ...
            print("Attack not complete or no results. Please run attack using 'run'")
            return
